...
function WelcomeScreen({ onStart }) {
  ...
  const validate = () => {
    ...
    if (!months || isNaN(months) || +months < 1 || +months > 60)
                                               e.months = "Enter months between 1 and 60.";
    return e;
  };
  ...
  const field = (label, value, setter, key, placeholder, type = "text") => (
    <div style={{ marginBottom: 16 }}>
      ...
      />
      {errors[key] && <div style={{ color: "#fca5a5", fontSize: 11, marginTop: 6, fontWeight: 600 }}>⚠️ {errors[key]}</div>}
    </div>
  );
...
function TrackerApp({ config, onReset }) {
  ...
  useEffect(() => { if (totalSaved >= GOAL && !celebrated) setCelebrated(true); }, [totalSaved]);

  const handleSave = () => {
    if (activeMonth === null || inputVal === "") return;
    const updated = [...saved]; updated[activeMonth] = inputVal;
    setSaved(updated); setInputVal(""); setActive(null);
  };
      ...
      <div style={{ maxWidth: 640, margin: "-16px auto 0", padding: "0 16px" }}>
        <div style={{ background: "white", borderRadius: 20, padding: 24, marginBottom: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.07)" }}>
          ...
          {celebrated && (
            <div style={{ marginTop: 16, background: "#f9fafb", border: "2px solid #6b7280", borderRadius: 14, padding: 14, textAlign: "center", fontSize: 15, fontWeight: 800, color: "#374151" }}>
              🎉 Amazing, {userName}! You've crushed your {fmt(GOAL)} goal!
            </div>
          )}
        </div>
        <div style={{ background: "white", borderRadius: 20, padding: 24, marginBottom: 16, boxShadow: "0 4px 20px rgba(0,0,0,0.07)" }}>
        ...
        <div style={{ textAlign: "center" }}>
          <div style={{ display: "inline-block", background: "linear-gradient(135deg, #111827, #374151)", borderRadius: 16, padding: "14px 28px" }}>
            <div style={{ fontSize: 10, color: "rgba(255,255,255,0.3)", letterSpacing: 1.5, textTransform: "uppercase", marginBottom: 4 }}>Designed & Built by</div>
